...
\subfloat[][Testing the main effect for the first factor (circle markers) against all possible combinations (without markers) with double cross-validation. The distributions only depend on the presence of the main effect. Note that there is no drift as in \autoref{fig:knn drift}.]{
\includegraphics[width=.48\textwidth]{sample_knn2_distributions_1}
...
\includegraphics[width=.48\textwidth]{sample_knn2_distributions_3}
...
%\end{subfigure}
\caption{Detection of main effects (a) and interaction (b). For the legend description see \autoref{fig:knn drift}. The power is worse than in \autoref{fig:knn convergence} due to smaller training set.}
\end{figure}
...
\bibliography{ref,../CliffordReport/ref}
